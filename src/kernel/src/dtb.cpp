                        ...
                        const prop_t *_prop)
{
    ...
    if ((_resource->type & resource_t::MEM) && (_resource->mem.len == 0))
    {
        ...
        else if (_node->parent->address_cells == 2)
        {
            ...
            _resource->mem.addr = be32toh(((uint32_t *)_prop->addr)[0]) + be32toh(((uint32_t *)_prop->addr)[1]);
            _resource->mem.len = be32toh(((uint32_t *)_prop->addr)[2]) + be32toh(((uint32_t *)_prop->addr)[3]);
        }
        else
        {
            assert(0);
...
bool DTB::dtb_init_cb(const iter_data_t *_iter, void *)
{
    ...
    case FDT_PROP:
    {
        ...
        nodes[idx].props[nodes[idx].prop_count].addr = (uintptr_t)(_iter->addr + 3);
...
bool DTB::dtb_init(void)
{
    ...
    // 内存保留区
    dtb_info.reserved = (fdt_reserve_entry_t *)(BOOT_INFO::boot_info_addr + be32toh(dtb_info.header->off_mem_rsvmap));
    // 数据区
    dtb_info.data = BOOT_INFO::boot_info_addr + be32toh(dtb_info.header->off_dt_struct);
    // 字符区
    dtb_info.str = BOOT_INFO::boot_info_addr + be32toh(dtb_info.header->off_dt_strings);
    // 检查保留内存
    dtb_mem_reserved();
    // 初始化 map
