...
bool PMM::init(void)
{
    ...
    size_t kernel_pages = (common::ALIGN(common::KERNEL_END_ADDR, common::PAGE_SIZE) - common::ALIGN(common::KERNEL_START_ADDR, common::PAGE_SIZE)) / common::PAGE_SIZE;
    // 将内核已使用部分划分出来
...
{
    size_t ret = kernel_space_allocator->get_used_count() + allocator->get_used_count();
    return ret;
}

size_t PMM::get_free_pages_count(void) const
{
    size_t ret = kernel_space_allocator->get_free_count() + allocator->get_free_count();
    return ret;
}
...
void PMM::free_page(uintptr_t _addr)
{
    ...
    else if (_addr >= non_kernel_space_start && _addr < non_kernel_space_start + non_kernel_space_length)
    {
