...
/// 页表或页目录（可以理解为数组指针），能通过下标访问每一个页表项
...
{
private:
    /// 页表项能映射多少页 = 页大小/页表项大小: 2^9
    static constexpr const size_t VMM_PAGES_PRE_PAGE_TABLE = common::PAGE_SIZE / sizeof(pte_t);
    /// 映射内核空间的大小
...
private:
     ...
     * @note 例如虚拟地址右移 12+(10 * _level) 位， 得到的就是第 _level 级页表的 VPN
     */
